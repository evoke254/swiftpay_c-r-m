    ...
    <div class="content">
        <div class="row d-flex justify-content-center">
          <div class="col-md-12 text-center">
            <h4>Generate {{requestparams.name}} </h4>
          </div>
        </div>
        <form action="#" @submit="formSubmit">
            <div class="row">
                <div v-for="(column, column_name) in requestparams.columns" class="col-md-6">
                    <div v-for="(display_label, column_type) in column" class="form-group row mb-5">
                        ...
                        <div class="col">
                            <datetimepicker v-if="column_type == 'datetime' || column_type == 'date'" v-model="quoteData[0][column_name]" class="form-control border border-light"></datetimepicker>
                             <textarea v-else-if="column_type == 'text'" class=" col-md-12 form-control rounded-4 border border-light"  rows="5" v-model="quoteData[0][column_name]"></textarea>
                            <model-select 
                            ...
                            <input  v-else-if="((column_type == 'bigint') && (display_label == 'Quote' || display_label == 'Invoice'))" type="text" class="form-control border border-light" disabled="" v-model="Quote">

                            <input v-else-if="column_type == 'string'" class="form-control border border-light" v-model="quoteData[0][column_name]" type="text" name="name" >
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row m-b-15 justify-content-center text-center">
                <div class="form-group row mb-5 ">
                    <button type="submit" class="btn btn-md btn-success">Generate</button>
                </div>
            </div>
        </form>
  ...
  export default {
    ...
    methods: {
      ...
      getselectedproduct() {
        ...
        formSubmit(e) {
        e.preventDefault();
          axios.post('/invoicestore', {
                Invoice_Subject: this.quoteData[0]['Invoice_Subject'],
                Invoice_Stage: this.quoteData[0]['Invoice_Stage'],
                ...
                Invoice_Stage: this.quoteData[0]['Invoice_Stage']['value'],  
                Organization: this.quoteData[0]['Organization']['value'],
                Description: this.quoteData[0]['Description'],
                Terms_and_Conditions: this.quoteData[0]['Description'],
