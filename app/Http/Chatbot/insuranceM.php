...
class insuranceM extends Conversation
{
    ...
    {
        $svc = InsuranceOrg::findOrFail(2)->getCompanyServices;
        $company = InsuranceOrg::findOrFail(2);
    ...
    public function compute_premium($service, $company, $imei, $supportChannel, $insurer, $estValue)
    {
        ...
        $this->ask("Your premium for \n *Huawei Y6 2017 [MYA-LII]* \n IMEI : *".$imei."* \n is Kshs. *".$premium_sum."* . \n Kindly respond with the MPESA number to receive an STK Push. Accepatble _Number format_ is *2547XXXXXXXX* ." , function(Answer $answer) use ($service, $company, $imei, $premium_sum, $supportChannel, $insurer){
                $mpesaNumber = $answer->getText();
                    if (preg_match('/^[2]{1}[5]{1}[4]{1}[0-9]{9}$/i', $mpesaNumber)){
                        $model = "Huawei Y6 2017";
